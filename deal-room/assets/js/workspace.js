...
(function () {
  ...
  function renderHeader() {
    ...
    const ccy = (window.DataBank && window.DataBank.currencyApi) ? window.DataBank.currencyApi().get() : "USD";
    ...
    return (
      '<header class="t-header" role="banner">' +
        ...
        '<div class="t-ccy-switch" role="group" aria-label="Display currency">' +
          '<button type="button" class="t-ccy-seg' + usdActive + '" data-set-ccy="USD" aria-pressed="' + (ccy === "USD") + '"><span>$</span><span class="t-ccy-code">USD</span></button>' +
          '<button type="button" class="t-ccy-seg' + ngnActive + '" data-set-ccy="NGN" aria-pressed="' + (ccy === "NGN") + '"><span>₦</span><span class="t-ccy-code">NGN</span></button>' +
        '</div>' +
